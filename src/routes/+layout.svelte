<script>
  ...
  // ⬅️ get server data from +layout.server.ts (already correct at SSR time)
  ...
  <header class="sticky top-0 z-50 bg-white/80 backdrop-blur border-b border-slate-200">
    <nav class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <div class="sm:hidden relative">
        ...
        {#if menuOpen}
          ...
          >
            ...
            {#if data.session}
              <a class="block rounded-lg px-3 py-2 hover:bg-slate-50" href="/logout" role="menuitem" onclick={() => (menuOpen = false)}>Logout</a>
            {:else}
				<a class="block rounded-lg px-3 py-2 hover:bg-slate-50" href="/login" role="menuitem" onclick={() => (menuOpen = false)}>Login</a>
              <a class="block rounded-lg px-3 py-2 hover:bg-slate-50" href="/register" role="menuitem" onclick={() => (menuOpen = false)}>Register</a>
            {/if}
          </div>
        {/if}
      </div>
      ...
      <ul class="hidden sm:flex items-center gap-1 text-sm">
        ...
        {#if data.session}
          <li><a class="rounded-lg px-3 py-2 hover:bg-slate-100" href="/logout">Logout</a></li>
        {:else}
		<li><a class="rounded-lg px-3 py-2 hover:bg-slate-100" href="/login">Login</a></li>
          <li><a class="rounded-lg px-3 py-2 hover:bg-slate-100" href="/register">Register</a></li>
        {/if}
      </ul>
    </nav>
  </header>
...
<style>
/* (optional) If you want a fixed min width where auth links render, uncomment:
   .auth-slot { min-width: 6rem; display:inline-block; } */
</style>
